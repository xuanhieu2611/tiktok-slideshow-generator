...
export async function exportSingleSlide(slide: Slide, index: number, width = CANVAS_WIDTH, height = CANVAS_HEIGHT): Promise<void> {
  try {
    const blob = await renderSlideToBlob(slide, width, height)
...
export async function exportAllAsZip(slides: Slide[], width = CANVAS_WIDTH, height = CANVAS_HEIGHT): Promise<void> {
  try {
    const zip = new JSZip()
    for (let i = 0; i < slides.length; i++) {
      try {
        const blob = await renderSlideToBlob(slides[i], width, height)
